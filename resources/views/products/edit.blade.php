        ...
        <div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
            ...
            <form action="{{ isset($product) ? route('products.update', $product->id) : route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="size" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="">Select Size</option>
                        <option value="Small" {{ (isset($product) && $product->size == 'Small') || old('size') == 'Small' ? 'selected' : '' }}>Small</option>
                        <option value="Medium" {{ (isset($product) && $product->size == 'Medium') || old('size') == 'Medium' ? 'selected' : '' }}>Medium</option>
                        <option value="Large" {{ (isset($product) && $product->size == 'Large') || old('size') == 'Large' ? 'selected' : '' }}>Large</option>
                        <option value="Extra Large" {{ (isset($product) && $product->size == 'Extra Large') || old('size') == 'Extra Large' ? 'selected' : '' }}>Extra Large</option>
                        <!-- You can add more cabinet sizes here -->
                    </select>
